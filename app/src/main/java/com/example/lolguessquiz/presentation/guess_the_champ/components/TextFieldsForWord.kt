...
fun TextFieldsForWord(
    ...
    if (letters <= 6) {
        Row(
            modifier = Modifier.padding(bottom = 16.dp), horizontalArrangement = Arrangement.Center
        ) {
            for (i in 1..letters) {
                MineBasicTextField(value = state.userWord.get(index = i - 1), onValueChanged = {
                    viewModel.onEvent(
                        GuessTheChampEvents.onTextChange(
                            it, index = i
                        )
                    )
                })
        ...
        TwoRowsForBiggerWords(
            letters1 = letters / 2, letters2 = letters / 2 + 1, state = state, viewModel = viewModel
        )
    } else {
        TwoRowsForBiggerWords(
            letters1 = letters / 2, letters2 = letters / 2, state = state, viewModel = viewModel
        )
    }
    ...
    for (i in state.lengthOfWord - 1 downTo 0) {
        LaunchedEffect(
            key1 = state.userWord[i]
        ) {
            if (state.userWord[i].isBlank()) {
                focusManager.moveFocus(
...
fun TwoRowsForBiggerWords(
    ...
    Row(
        ...
        for (i in 1..letters1) {
            MineBasicTextField(value = state.userWord.get(index = i - 1), onValueChanged = {
                viewModel.onEvent(GuessTheChampEvents.onTextChange(it, index = i))
            })
        }
    }
    Row(
        ...
        for (i in 1..letters2) {
            MineBasicTextField(value = state.userWord.get(index = (letters1 + i - 1)),
                onValueChanged = {
                    viewModel.onEvent(
                        GuessTheChampEvents.onTextChange(
                            it, index = (letters1 + i)
